...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const allowed = ["name","description","category","status","icon","parametersSchema","credentialsSchema","n8n","pricing","activationRules","features"];
    const patch = { updatedAt: admin.firestore.FieldValue.serverTimestamp() };
    allowed.forEach(k => { if (updates[k] !== undefined) patch[k] = updates[k]; });
    if (updates.n8n) patch.n8n = { ...snap.data().n8n, ...updates.n8n };
